...
if __name__ == '__main__':
    ...
    # 读取数据
    Capacity = pd.read_csv('./LSTM模型时序数据/capacity_discharge_B0006.csv', header=None).iloc[:, 0].tolist()
    HI = pd.read_csv('./LSTM模型时序数据/HI_B0006.csv', header=None).iloc[:, 0].tolist()
    ...
    #     pivot.append([res[0]])
    #     with open('batch_size_B0006.csv', 'a', encoding='utf-8') as csvFile:
    ...
    #     pivot.append([res[0]])
    #     with open('neuron_B0006.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(neuron, res[0])
    ...
    #     res = train_model(HI, Capacity, timeStep, epoch, bathSize, neuron)
    #     pivot.append([res[0]])
    #     with open('epoch_B0006.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(epoch, res[0])
    ...
    # print(modelRMSE)
    # 1层，0.027818298387315983
    # 2层，0.028715165534146065
    # 3层，0.0316348652092888
    # 4层，0.03532894961621458
    # 1层时最小
    ...
    #     with open('timestep_B0006.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    ...
    for SP in SPs:
        ...
        tru = Capacity[SP-1:]
        ##存储预测结果
        with open('./LSTM预测原始数据/lstm_pre_B0006_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
        ...
        with open('./LSTM预测原始数据/lstm_rmse_mae_r2_B0006_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
