...
if __name__ == '__main__':
    ...
    SOH = (Capacity/np.max(Capacity)).tolist()
    ...
    for SP in SPs:
        ...
        # 当监测到loss停止改进时，结束训练。patience=2表示经过2个周期结果依旧没有改进，此时可以结束训练。
        early_stop = EarlyStopping(monitor='loss', patience=2, verbose=1)
        modelRMSE, lstmModel = train_model(HI, SOH, timeStep, epochs, bathSize, neuron)
        print('modelRMSE:', modelRMSE)

        # 预测
        data = HI[SP-timeStep-1:]
        Tru = SOH[SP-timeStep-1:]
        test = scale(data)
        ...
        tru = SOH[SP-1:]
        # 存储预测结果
        with open('./LSTM预测原始数据/lstm_pre_162SOH_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
        ...
        with open('./LSTM预测原始数据/lstm_rmse_mae_r2_162SOH_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
