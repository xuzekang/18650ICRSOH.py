...
def buildLSTM(timeStep, neuron):
    ...
    model.add(LSTM(neuron, return_sequences=True,input_shape=(timeStep, 1), activation='tanh'))  # 添加LSTM层
...
if __name__ == '__main__':
    ...
    # 读取数据
    Capacity = pd.read_csv('./LSTM模型时序数据/capacity_discharge_B0005.csv', header=None).iloc[:, 0].tolist()
    HI = pd.read_csv('./LSTM模型时序数据/HI_B0005.csv', header=None).iloc[:, 0].tolist()
    ...
    #     pivot.append([res[0]])
    #     with open('batch_size_B0005.csv', 'a', encoding='utf-8') as csvFile:
    ...
    #     pivot.append([res[0]])
    #     with open('neuron_B0005.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(neuron, res[0])
    ...
    #     res = train_model(HI, Capacity, timeStep, epoch, bathSize, neuron)
    #     pivot.append([res[0]])
    #     with open('epoch_B0005.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(epoch, res[0])
    ...
    # print(modelRMSE)
    # 1层，0.03375856782678831
    # 2层，0.018167761461088384
    # 3层，0.018581707997793778
    # 4层，0.02093760030140297
    # 2层时最小
    ...
    #     with open('timestep_B0005.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    ...
    for SP in SPs:
       ...
       ## 预测
        ...
        tru = Capacity[SP-1:]
      ##存储预测结果
        with open('./LSTM预测原始数据/lstm_pre_B0005_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
        ...
        res.append(math.sqrt(MAE(tru, pre)))
        res.append(math.sqrt(R2(tru, pre)))
        with open('./LSTM预测原始数据/lstm_rmse_mae_r2_B0005_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
