...
def buildLSTM(timeStep, neuron):
    ...
    model.add(LSTM(neuron, input_shape=(timeStep, 1), activation='tanh'))  # 添加LSTM层
...
if __name__ == '__main__':
    ...
    # 读取数据
    Capacity = pd.read_csv('./LSTM模型时序数据/capacity_discharge_B0007.csv', header=None).iloc[:, 0].tolist()
    HI = pd.read_csv('./LSTM模型时序数据/HI_B0007.csv', header=None).iloc[:, 0].tolist()
    ...
    #     pivot.append([res[0]])
    #     with open('batch_size_B0007.csv', 'a', encoding='utf-8') as csvFile:
    ...
    #     pivot.append([res[0]])
    #     with open('neuron_B0007.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(neuron, res[0])
    ...
    #     res = train_model(HI, Capacity, timeStep, epoch, bathSize, neuron)
    #     pivot.append([res[0]])
    #     with open('epoch_B0007.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    #     print(epoch, res[0])
    ...
    # print(modelRMSE)
    # 1层，0.018264477504277187
    # 2层，0.01930014835528309
    # 3层，0.021033794347400978
    # 4层，0.023476930049068433
    # 1层时最小
    ...
    #     with open('timestep_B0007.csv', 'a', encoding='utf-8') as csvFile:
    #         np.savetxt(csvFile, np.array(pivot).T, delimiter=',', fmt='%s')
    #     pivot = list()
    ...
    for SP in SPs:
        ...
        tru = Capacity[SP-1:]
        # 存储预测结果
        with open('./LSTM预测原始数据/lstm_pre_B0007_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
        ...
        with open('./LSTM预测原始数据/lstm_rmse_mae_r2_B0007_%s.csv' % SP, 'a', encoding='utf-8') as csvFile:
